...
//画像回転
void iex2DObj::RenderI(s32 DstX, s32 DstY, s32 DstW, s32 DstH, s32 SrcX, s32 SrcY, s32 width, s32 height, float rad, u32 dwFlags, COLOR color, float z)
{
	int OriginX = DstX + (DstW / 2);                                        //  中心の座標X
	int OriginY = DstY + (DstH / 2);                                        //  中心の座標Y
	float OriginD = sqrtf((DstW / 2)*(DstW / 2) + (DstH / 2)*(DstH / 2));   //  中心から各点までの距離
	float CentralRad = atanf((2 * ((float)DstH / 2)) / (float)DstW);        //  基準線(中心からX正方向に水平に延びる線)と中心と点s[1](右上)を結ぶ直線との反時計回りの回転角
	float OriginRad = PI + CentralRad;                                      //  基準線と点s[0]との時計回りの回転角
	CentralRad = PI - CentralRad * 2;                                       //  ここで中心と点s[0]・点s[1]を結ぶ直線との時計回りの回転角に変換      
	...
	v[1].sx = v[3].sx = (float)(DstX + DstW) - 0.5f;
	v[0].sy = v[1].sy = (float)DstY;
	v[2].sy = v[3].sy = (float)(DstY + DstH) - 0.5f;  //  元データ

	v[0].sx = OriginX + OriginD*cosf(OriginRad + rad);
	v[1].sx = OriginX + OriginD*cosf(OriginRad + rad + CentralRad);
	v[2].sx = OriginX + OriginD*cosf(OriginRad + rad + PI + CentralRad);
	v[3].sx = OriginX + OriginD*cosf(OriginRad + rad + PI);

	v[0].sy = OriginY + OriginD*sinf(OriginRad + rad);
	v[1].sy = OriginY + OriginD*sinf(OriginRad + rad + CentralRad);
	v[2].sy = OriginY + OriginD*sinf(OriginRad + rad + PI + CentralRad);
	v[3].sy = OriginY + OriginD*sinf(OriginRad + rad + PI);
