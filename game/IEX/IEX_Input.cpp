...
void iexInput::SetInfo()
{
	...
	if( lpDevice ){
		...
		if( lpDevice->GetDeviceState(sizeof(DIJOYSTATE2),&js) == DI_OK ){
			...
			PadAxisX3 = axis[JoyMap[16]];	//	右スティックＸ軸
			PadAxisY3 = axis[JoyMap[17]];	//	右スティックＸ軸
...
int KEY_GetAxisX3() { return input->Get(KEY_AXISX3); }
int KEY_GetAxisY3() { return input->Get(KEY_AXISY3); }
